...
@Component
public class ThumbnailGalleryGenerator {
	...
	private File writeHtmlToTempFile(String htmlAsString) {
		...
		try {
			tmpFile = Files.createTempFile("thumbnailGallery", ".html").toFile();
	...
	private String generateHtml(UserCollection collection) {
		...
		String htmlAsString = templateEngine.process("thumbnailGallery.html", ctx);
		return htmlAsString;
	}
	...
	private Predicate<? super CollectionItem> hasAnImage() {
		return item -> !StringUtils.isEmpty(item.getImageUrl()) && !StringUtils.isEmpty(item.getThumbnailUrl());
	}
}
